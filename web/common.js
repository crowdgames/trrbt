...
function find_file_node_ids(file, node, resolve_file_to_game, nid_to_node) {
    ...
    if (node.hasOwnProperty('children')) {
        for (let child of node.children) {
            find_file_node_ids(file, child, resolve_file_to_game, nid_to_node);
        }
    }
    if (node.hasOwnProperty('file') && node.hasOwnProperty('target')) {
        if (resolve_file_to_game != null) {
            ...
            if (game_tree) {
                find_file_node_ids(node.file, game_tree, resolve_file_to_game, nid_to_node);
            }
        }
    }
}
...
function xform_apply_to_node(node, xforms, nid_to_node, already_linked, apply_xform, dispid_use_or_prefix) {
    let ret_nodes = [];
        ...
        if (node.hasOwnProperty('target')) {
            ...
            if (target) {
                ...
                if (already_linked.indexOf(linked_id) >= 0) {
                    ...
                    const linked = xform_apply_to_node(deepcopyobj(target), xforms, nid_to_node, [linked_id].concat(already_linked), apply_xform, linked_dispid_suffix);
                    return linked;
                }
            }
        }
    ...
    if (!apply_xform) {
        ...
        for (let ret_node of ret_nodes) {
            if (ret_node.hasOwnProperty('children')) {
                ...
                for (const child of ret_node.children) {
                    const child_xformed = xform_apply_to_node(child, xforms, nid_to_node, already_linked, apply_xform, dispid_use_or_prefix)
        ...
        if (['x-link', 'x-file'].indexOf(ntype) >= 0) {
            ...
            for (const child of node.children) {
                let dispid_suffix = 0;
                const children_xformed = xform_apply_to_node(child, [fn].concat(xforms), nid_to_node, already_linked, apply_xform, dispid_use_or_prefix)
...
function xform_apply_to_tree(tree, resolve_file_to_game, apply_xform, use_dispids) {
    ...
    find_file_node_ids(null, tree, resolve_file_to_game, nid_to_node);

    return xform_apply_to_node(tree, [xform_rule_identity], nid_to_node, [], apply_xform, use_dispids ? null : undefined)[0];
}
