...
for game in `ls games/trees | grep '\.json$'`; do
    ...
    bash script/game_cvt.sh --game games/trees/${game}.json --out viz/gv/${game}--unxform.gv --fmt gv --resolve
    bash script/game_cvt.sh --game games/trees/${game}.json --out viz/gv/${game}--xform.gv --fmt gv --resolve --xform
